...
module.exports = {
  buildModel: (model) => (req, res, next) => {
    let modelName =
      model || modelMaps.getModelFromRoute(req.params.model, req.method);
    let m = db[modelName];
  ...
  model: (model) => (req, res, next) => {
    const modelName = model || modelMaps.route(req.params.model, req.method);
        ...
        if (req.query.hasOwnProperty(item.fieldName)) {
          ...
          switch (item.type.key) {
            ...
            case "FLOAT":
              ...
              val =
                value.length === 1 &&
                (dataTypes.isNullValue(value[0]) ||
                  value[0].trim().toLowerCase() === "undefined")
                  ? null
                  : value;
              break;
            case "DATE":
            case "DATEONLY":
              ...
              if (values.length > 1) {
                val = values.map((it) =>
                  dataTypes.isNullValue(it) ? null : moment(it)
                );
              } else {
                val = dataTypes.isNullValue(values[0])
                  ? null
                  : moment(values[0]);
              }
              break;
  ...
  include: (model) => (req, res, next) => {
    const modelName = model || modelMaps.route(req.method, req.params.model);
    if (!modelName) return next();
    ...
    if (!!attributes.length) {
      for (let i = 0; i < attributes.length; i++) {
        ...
        if (currentAttribute.length > 1) {
          const assocArray = currentAttribute.slice(
            0,
          ...
          for (let i = 0; i < assocArray.length; i++) {
            ...
            if (!!a && a.include) include = a.include;
          }
          if (!a) continue;
  ...
  order: (model) => (req, res, next) => {
    ...
    const isAttrCreatedAt =
      rawAttributesKeys.findIndex((attr) => attr === "createdAt") > -1;
    ...
    if (!!rawOrder) {
      order = rawOrder.split(",").map((row) => {
        ...
        return [db.Sequelize.col(items[0]), items[1]];
      });
    } else if (
      isAttrCreatedAt &&
      !queryAttrCreatedAt &&
   ...
   */
  not: () => (req, res, next) => {
    if (!req.query.not) return next();
    const not = {};
    _.forEach(req.query.not, (v, k) => {
      const value = v.split(",") || [];
      not[k] =
        value.length === 1 && value[0].trim().toLowerCase() === "null"
          ? null
          : value;
    });
    req.query.not = not;
